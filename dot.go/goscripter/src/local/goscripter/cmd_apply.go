...
func CmdApply(argv []string) int {
	...
	autoYes := fs.Bool("y", FalseDefault(), "assume yes; do not prompt")
	verbose := fs.Bool("verbose", FalseDefault(), "verbose output")
	...
	args := fs.Args()
	if len(args) != 1 {
		eprintf("apply: script.go required")
		return 2
	}
	script := args[0]
	...
	if needShebang {
		allowed := *autoYes || askConfirm(fmt.Sprintf("Add/normalize shebang on %s?", abs), false)
		if !allowed {
			if *verbose {
	...
	if needsExec {
		allowed := *autoYes || askConfirm(fmt.Sprintf("Add owner-exec bit (chmod u+x) on %s?", abs), false)
		if allowed {
